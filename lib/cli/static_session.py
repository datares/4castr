...
def train_val_test_split(path):
    ...
    Returns:
        train_data, validation_data, test_data (tuple): Pandas DataFrames containing split data
    """
...
class Static_Session:
    ...
    def optimize_params(self,
        ...
        Args:
            trial (optuna.trial): Trial object to generate hyperparameter values.
            n_prune_evals_per_trial (int): Number of times to evaluate the model and prune if necessary. Defaults to 2.
            n_tests_per_eval (int): Unused.
            model_params (dict): Dictionary of hyperparameters generated by optimize_agent_params. Defaults to self.optimize_agent_params(trial).
            train_env (stable_baselines.common.vec_env.DummyVecEnv): training environment on which PPO2 is trained. Defaults to a SimulatedEnv.
            model(stable_baselines model): Model to optimize parameters for. Defaults to PPO2.
    ...
    def run_optimization(self, n_trials: int = 10):
        """
        Runs a hyperparameter search and selects the best based on highest reward.
        
        Args:
            n_trials (int): number of trials to run. Each a unique set of hyperparameters. Defaults to 10.
    ...
    def run_test(self, model,
        ...
        Args:
            ...
            validation(bool): Whether or not the model is to be validated on validation data. Defaults to True.
            finetune(bool): Whether or not the model is to be tested on external dataset. Defaults to False.
            out_file(bool): Whether or not to write model stats to output file. Defaults to False.
            verbose(bool): Whether or not to print model stats. Defaults to True.
    ...
    def run_train(self):
        """
        Trains a PPO2 algorithm on stock trading environment SimulatedEnv. Saves the model via pickle when finished.
