...
class LiveEnv():
    ...
    def _step(self, action):
        ...
        info = {'cur_val': cur_val, 'prev_val': prev_val, 'action': action, 'crypto_owned': self.crypto_owned,
                'cash_in_hand': self.cash_in_hand, 'price': self.open_price}
    ...
    def _get_obs(self):
        obs = []
        data = get_live_OHLCV()
        live_ta = add_all_ta_features(data, "price_open", "price_high", "price_low", "price_close", "volume_traded",
                                      fillna=True)
